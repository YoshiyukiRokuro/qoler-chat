...
export default {
  ...
  setup(props, { emit }) {
    ...
    // channelId propから最新のチャンネル情報を取得
    const channel = computed(() =>
      store.state.channels.find((c) => c.id === props.channelId)
    );

    const allUsers = computed(() => store.getters.allUsers);
    const currentMembers = computed(
      () => store.state.channelMembers[props.channelId] || []
    );
    ...
    const updateName = async () => {
      try {
        await store.dispatch("updateChannelName", {
        ...
        toast.error(
          error.response?.data?.error || "名前の変更に失敗しました。"
        );
      }
    };
    ...
    const addMember = async () => {
      ...
      try {
        await store.dispatch("addMembersToChannel", {
          channelId: props.channelId,
          userIds: [selectedUserToAdd.value],
        });
    ...
    const removeMember = async (userId) => {
      ...
      try {
        await store.dispatch("removeMembersFromChannel", {
          channelId: props.channelId,
          userIds: [userId],
        });
